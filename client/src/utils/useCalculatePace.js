...
// T₁ — время на известной дистанции
// D₁ — известная дистанция
// D₂ — целевая дистанция;
// T₂ — прогнозируемое время;
// 1.06 — коэффициент замедления (усталости)

// На 5 км темп обычно быстрее на 2-4% (около
//  секунд на км), так как дистанция короче в два раза.
// На полумарафоне темп замедляется примерно на 3-4% (
//  секунд к темпу «десятки»).
// На марафоне темп падает значительнее — примерно на 8% (
//  секунд к темпу «десятки»)

import { useDispatch } from 'react-redux'

import { setCustomPace } from '../redux/slices/customPlanSlice'

const useCalculatePace = () => {
  const dispatch = useDispatch()

  // ф-я расчета темпа аргументы в метра и секундах
  ...
  //для расчета дистанция должна быть в метрах,а время в секундах
  const countRunPaceFormula = (distanceUser, timeUser) => {
    ...
    dispatch(setCustomPace(paceObj))
    return paceObj
  }
...
export default useCalculatePace
