...
import { workoutPool } from '../data/workoutPool.js'
import { getLoadStatus } from './getLoadStatus.js'
import {
  multipliers,
  maxLongRunLimits,
  minLongRunLimits,
} from '../data/multipliers.js'
import { generateTitlePlan } from './generateTitlePlan.js'
import { generateWorkoutDescription } from './generateWorkoutDescription.js'
import { countRunPaceFormula } from './paceCalculator.js'
...
const generateRunningPlan = ({
  ...
  const structures = {
    3: ['rest', 'intervals', 'rest', 'tempo', 'rest', 'rest', 'long'],
    4: ['rest', 'intervals', 'easy', 'rest', 'tempo', 'rest', 'long'],
    5: ['rest', 'intervals', 'easy', 'tempo', 'rest', 'easy', 'long'],
    6: ['easy', 'intervals', 'easy', 'tempo', 'easy', 'rest', 'long'],
    7: ['easy', 'intervals', 'easy', 'tempo', 'easy', 'easy', 'long'],
  }
  ...
  for (let w = 1; w <= weeks; w++) {
    ...
    structure.forEach((type, inx) => {
      ...
      let options = (workoutPool[type] || []).filter((work) => {
        ...
        const levelMatch = !work.levels || work.levels.includes(level)
